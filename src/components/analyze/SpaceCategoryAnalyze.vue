...
import {getPictureCategoryAnalyzeUsingPost} from "@/api/spaceAnalyzeController";
...
const fetchData = async () => {
  loading.value = true;
  const res = await getPictureCategoryAnalyzeUsingPost({
...
const options = computed(() => {
  const categories = dataList.value.map((item) => item.category ?? '无分类')
